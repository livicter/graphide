...
15. **Harness** — `npm install && npx playwright install --with-deps chromium && npm run verify`
   ...
   - WHOOP health chrome: explorer Night Map; three `#coverage .dial`
     rings Coverage / Health / Open; WHOOP tokens; raised `.bubble-card`;
     UPPERCASE `.dial .label`; Day hides `.dials`; Map `xy=0`;
     `verification/whoop-health-night.png`
...
16. **Evidence** — stdout prints a `PASS verify-graphide` line that **mentions
    ...
    **path-walk**, **appearance**, **apple-chrome**, **apple-chrome-icons**, **whoop-health**, **coverage-mark**, **hop-card**, **fit-reorg**,
...
| WHOOP health chrome | `#coverage .dials .dial[data-dial]`, `.dial .ring circle.arc`, `.dial .label`, `.bubble-card`, `#themeNight` |
