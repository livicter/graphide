...
11. **JavaScript desk fixture** — prove the compiled-in `javascript@` deriver on
    the Review desk (not `plugins --check` alone):
    ...
    Snap `plugin` matches `javascript@`; nodes / edges / files `> 0`; a flow
    has Source and Sink (data-subscription: publish → events → subscribe).
...
14. **Package** — `npm ci --prefix extension && npm run package` writes `extension/graphide-*.vsix` and must pass `npm run check:package` plus `npm run check:activation`. The VSIX holds compiled `out/extension.js`, `media/main.js`, `media/main.css`, `media/xyflow.css`, `media/icon.svg`, and `bin/graphide` (or `graphide.exe`) for this host. It must not contain `media/src/` or `extension/src/`. Core Review does not need an LLM key. This does **not** launch a VS Code Extension Host. `check:activation` loads `extension/out/review-folder.js` and requires `reviewFolder("  /opt/pkg  ", "/ws") === "/opt/pkg"`, `reviewFolder("", "/ws/app") === "/ws/app"`, and a throw of `Open a workspace folder` when both are empty. `graphide.review` must call `runReview`, and `packageRoot` must call `reviewFolder`. `.cursor/rules/prefer-herd.mdc` is `alwaysApply: true` and `AGENTS.md` links it. The checker writes `verification/plugin-packaging.txt`.
