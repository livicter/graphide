...
- Python desk snap: `graphide review --root fixtures/python --json --no-parent`. Compiled-in `python@` on the Review desk (`?python=1`).
- JavaScript desk snap: `graphide review --root fixtures/js --json --no-parent`. Compiled-in `javascript@` on the Review desk (`?js=1`).
- TypeScript desk snap: `graphide review --root fixtures/ts --json --no-parent`. Compiled-in `typescript@` on the Review desk (`?ts=1`).
- Lifecycle snap (review machine): same demo derive. proposed → walking → stamped | skipped | broken; recover is `broken → walking`.
