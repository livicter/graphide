...
assert(harness.includes("js-snap.json") && harness.includes("__graphideJs"), "javascript fixture snap loader missing");
assert(harness.includes("ts-snap.json") && harness.includes("__graphideTs"), "typescript fixture snap loader missing");
...
assert(fs.existsSync(path.join(__dirname, "../../.cursor/skills/verify-graphide/references/features/panel-timeout.md")), "panel-timeout feature map missing");
...
assert(featReadme.includes("js-desk.md") && featReadme.includes("js-desk.png"), "feature README must list js-desk.md");
...
assert(featReadme.includes("delta-sticky-views.md") && featReadme.includes("delta-sticky-views.png"), "feature README must list delta-sticky-views.md");
...
const stickyViewsMap = fs.readFileSync(path.join(__dirname, "../../.cursor/skills/verify-graphide/references/features/delta-sticky-views.md"), "utf8");
assert(
  stickyViewsMap.includes("## Sub-features") &&
    stickyViewsMap.includes("## How to get to it (user POV)") &&
    stickyViewsMap.includes("## Driving it with the harness") &&
    stickyViewsMap.includes("## Gotchas"),
...
const deltaOnAnalysisMap = fs.readFileSync(path.join(__dirname, "../../.cursor/skills/verify-graphide/references/features/delta-onanalysis.md"), "utf8");
assert(
  deltaOnAnalysisMap.includes("## Sub-features") &&
    deltaOnAnalysisMap.includes("## How to get to it (user POV)") &&
...
const panelTimeoutMap = fs.readFileSync(path.join(__dirname, "../../.cursor/skills/verify-graphide/references/features/panel-timeout.md"), "utf8");
assert(
  panelTimeoutMap.includes("## Sub-features") &&
    panelTimeoutMap.includes("## How to get to it (user POV)") &&
    panelTimeoutMap.includes("## Driving it with the harness") &&
    panelTimeoutMap.includes("## Gotchas"),
  "panel-timeout feature map must use the four headings"
);
...
const allProgramsMap = fs.readFileSync(path.join(__dirname, "../../.cursor/skills/verify-graphide/references/features/all-programs.md"), "utf8");
assert(
  allProgramsMap.includes("## Sub-features") &&
    allProgramsMap.includes("## How to get to it (user POV)") &&
...
assert(fs.existsSync(path.join(__dirname, "../../.cursor/skills/verify-graphide/references/features/js-desk.md")), "js-desk feature map missing");
assert(fs.existsSync(path.join(__dirname, "../../.cursor/skills/verify-graphide/references/features/ts-desk.md")), "ts-desk feature map missing");
...
assert(workflow.includes("js-snap.json") && workflow.includes("fixtures/js"), "CI must derive fixtures/js for the JavaScript desk");
assert(workflow.includes("ts-snap.json") && workflow.includes("fixtures/ts"), "CI must derive fixtures/ts for the TypeScript desk");
...
assert(driver.includes("delta-sticky-views.png") && driver.includes("SV0") && driver.includes("parent_bubbles") && driver.includes("#deltaCanvas .vnode[data-bubble]"), "verify driver must prove sticky community members on Before/After");
...
assert(driver.includes("js-desk.png") && driver.includes("JS_HARNESS") && driver.includes("JS0") && driver.includes("javascript@"), "verify driver must drive the JavaScript desk on fixtures/js");
assert(driver.includes("ts-desk.png") && driver.includes("TS_HARNESS") && driver.includes("TS0") && driver.includes("typescript@"), "verify driver must drive the TypeScript desk on fixtures/ts");
...
assert(/panelOnly && mapStageMounted\(\)/.test(js) && /queuePanelRefresh\(\)/.test(js), "coverage-only patch must not paint a mounted Map");
assert(ext.includes("canPushPanelPatch") && ext.includes("pushPanelPatch") && /type:\s*"patch"/.test(ext), "host must post patch when graph/bubbles/flows are unchanged");
...
assert(js.includes('data-prog="-1"') && js.includes("All programs"), "desk must paint All programs chip");
...
const jsDeskMap = fs.readFileSync(path.join(__dirname, "../../.cursor/skills/verify-graphide/references/features/js-desk.md"), "utf8");
assert(
  jsDeskMap.includes("## Sub-features") &&
    jsDeskMap.includes("## How to get to it (user POV)") &&
    jsDeskMap.includes("## Driving it with the harness") &&
    jsDeskMap.includes("## Gotchas"),
  "js-desk feature map must use the four headings"
);
const tsDeskMap = fs.readFileSync(path.join(__dirname, "../../.cursor/skills/verify-graphide/references/features/ts-desk.md"), "utf8");
assert(
  tsDeskMap.includes("## Sub-features") &&
    tsDeskMap.includes("## How to get to it (user POV)") &&
    tsDeskMap.includes("## Driving it with the harness") &&
    tsDeskMap.includes("## Gotchas"),
  "ts-desk feature map must use the four headings"
);
const appleChromeMap = fs.readFileSync(path.join(__dirname, "../../.cursor/skills/verify-graphide/references/features/apple-chrome.md"), "utf8");
assert(
  appleChromeMap.includes("## Sub-features") &&
    appleChromeMap.includes("## How to get to it (user POV)") &&
    appleChromeMap.includes("## Driving it with the harness") &&
    appleChromeMap.includes("## Gotchas"),
  "apple-chrome feature map must use the four headings"
);
const appleChromeIconsMap = fs.readFileSync(path.join(__dirname, "../../.cursor/skills/verify-graphide/references/features/apple-chrome-icons.md"), "utf8");
assert(
  appleChromeIconsMap.includes("## Sub-features") &&
    appleChromeIconsMap.includes("## How to get to it (user POV)") &&
    appleChromeIconsMap.includes("## Driving it with the harness") &&
...
const jsFlows = fs.readFileSync(path.join(__dirname, "../../fixtures/js/flows.toml"), "utf8");
assert(/name\s*=\s*"data-subscription"/.test(jsFlows) && /pkg\.sub\.subscribe/.test(jsFlows), "js flows.toml must keep the data-subscription hit list");
const tsFlows = fs.readFileSync(path.join(__dirname, "../../fixtures/ts/flows.toml"), "utf8");
assert(/name\s*=\s*"data-subscription"/.test(tsFlows) && /pkg\.sub\.subscribe/.test(tsFlows), "ts flows.toml must keep the data-subscription hit list");
...
assert(
  chrome.includes("parent_bubbles") &&
    chrome.includes("function communityWantFqns") &&
    chrome.includes("function parentBubbles"),
  "desk must resolve community facts via parent_bubbles"
);
...
assert(/html\.bright #reviewBtn[\s\S]{0,80}#007aff/i.test(css), "bright Review button must stay system blue");
assert(/html\.bright \.workspaces button\.on[\s\S]{0,80}#007aff/i.test(css), "selected workspace chip must be system-blue tinted");
assert(/html\.bright #coverage[\s\S]{0,220}backdrop-filter/.test(css), "coverage strip must frost");
assert(css.includes(".bubble-card .tile"), "Map cards must style a tinted tile");
